...
class DiaryApi:
    def __init__(self, session: ClientSession, sessionid: str, user_information: dict):
    ...
    async def _post(
        ...
        async with self._session.post(f"https://sosh.mon-ra.ru/rest/{endpoint}", data=data) as r:
            json = await _check_response(r, self._session)
            return cls.reformat(json)

    @classmethod
    async def auth_by_diary_session(
        cls, diary_session: str, diary_information: dict
    ) -> "DiaryApi":
    ...
    async def auth_by_login(cls, login: str, password: str) -> "DiaryApi":
        ...
        async with session.get(
            f"https://sosh.mon-ra.ru/rest/login?" f"login={login}&password={password}"
        ) as r:
    ...
    async def check_food(self) -> types.CheckFoodObject:
        ...
        async with self._session.post("https://sosh.mon-ra.ru/rest/check_food") as r:
            json = await _check_response(r, self._session)
